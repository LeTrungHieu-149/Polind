...
function scrollFunction() {
    ...
    pendingElement.forEach(function (value) {
        if (value.className.includes("to-top")) {
            if (value.getBoundingClientRect().top - 650 <= 0)
                value.classList.add("reveal");
        } else if (value.getBoundingClientRect().top - 500 <= 0)
            value.classList.add("reveal");
        else return;
    });
}
...
window.addEventListener("click", function (event) {
    if (
        !event.target.classList.contains("header__menu") &&
